...
export async function createAgent(params: {
  ...
  const result = await pool.query(
    `INSERT INTO owned_agents (user_id, name, description, traits, system_prompt, model, avatar_color, source_type)
...
export async function updateAgent(
  id: string,
  userId: string,
  params: Partial<Pick<OwnedAgent, "name" | "description" | "traits" | "systemPrompt" | "model" | "avatarColor">>
  ...
  if (params.traits !== undefined) { setClauses.push(`traits = $${idx++}::jsonb`); values.push(JSON.stringify(params.traits)); }
